...
const stateCreator: StateCreator<MapState & MapActions> = (set, get) => {
  const actions: MapActions = {
    ...
    updateNodeData: (nodeId, data) => {
      ...
      // Only track as updated if it's not a newly added node
    ...
    deleteNode: (nodeId) => {
      const { nodes, edges, changes } = get();
      const newNodes = nodes.filter((n) => n.id !== nodeId);
      const newEdges = edges.filter((e) => e.source !== nodeId && e.target !== nodeId);
      
      // Track deletion
      if (changes.addedNodes.has(nodeId)) {
        // If it was newly added, just remove from added list
        changes.addedNodes.delete(nodeId);
      } else {
        // If it existed before, mark as deleted
